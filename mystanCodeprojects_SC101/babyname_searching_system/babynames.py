...
def add_data_for_name(name_data, year, rank, name):
    ...
    """
    if name not in name_data:  # 加入新名字
        name_data[name] = {}  # 先讓新的name變成一個字典
        name_data[name][year] = rank  # d:name_data[name],[key]:[year] = value
    elif year not in name_data[name]:  # 有名字沒年分
        name_data[name][year] = rank  # 加入年分
    else:  # 兩者都有
        existing_rank = int(name_data[name][year])
        new_rank = int(rank)  # 輸入的新資料直接找到rank對應的排名就好
